...
def merge_k_lists(matrix):
    ...
    row = column = 0
    length = len(matrix)
    while column == 0 and row < 10:
        temp_lst.append(matrix[row][column])
        row += 1
        
    print(temp_lst)
    while len(sorted_lst) < 1000:
        minimum = min(temp_lst)
        temp_index = temp_lst.index(minimum)
        sorted_lst.append(minimum)
        if column <= 99 and row < 9:
            row += 1
        elif column == 99 and row == 9:
            pass
        else:
            column += 1
            row = 0
            temp_lst[temp_index] = matrix[row][column]
